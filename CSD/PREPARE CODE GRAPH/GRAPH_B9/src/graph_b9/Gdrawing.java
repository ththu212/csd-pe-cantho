...
//    }
    ...
    ArrayList<Edges> ds = new ArrayList<>();

    public void Edges() {
        for (int i = 0; i < NumberOfVertices; i++) {
            for (int j = i + 1; j < NumberOfVertices; j++) {
                ds.add(new Edges(i, j, graph[i][j]));
            }
        }
    }
    ...
    int graphClone[][];
    boolean isVisited2[];
    String resultCutEdges = "";
    int dem = 0;

    public void ResetCutEdges() {
        graphClone = new int[NumberOfVertices][NumberOfVertices];
        for (int i = 0; i < NumberOfVertices; i++) {
            for (int j = 0; j < NumberOfVertices; j++) {
                graphClone[i][j] = graph[i][j];
            }
        }
        isVisited2 = new boolean[NumberOfVertices];
        for (int i = 0; i < NumberOfVertices; i++) {
            isVisited2[i] = false;
        }
    }
    ...
    public void CutEdges() {
        for (Edges i : ds) {
            ResetCutEdges();
            graphClone[i.getFrom()][i.getTo()] = 0;
            graphClone[i.getTo()][i.getFrom()] = 0;
            if (CountComponent() > 1) {
                dem++;
                resultCutEdges += "\n" + i.getFrom() + " " + i.getTo();
            }
    ...
    public void BFS(int startVertex) {
        TraversalReset();
        isVisited2[startVertex] = true;
        int from;
        q.add(startVertex);
        while (!q.isEmpty()) {
            from = q.poll();
            for (int i = 0; i < NumberOfVertices; i++) {
                if (isVisited2[i] == false && graphClone[from][i] != 0) {
                    isVisited2[i] = true;
                    q.add(i);
                }
            }
    ...
    public int getCountCutEdge() {
        return dem;
    }

    public String getCountEdgeResult() {
        return resultCutEdges;
    }

//    String str = "";
//
//    public void PrimDisplay() {
//        int from, to;
//        int edgeIndex;
//        for (int i = 0; i < NumberOfVertices; i++) {
//            to = i;
...
class Edges {
    ...
    public Edges(int from, int to, int value) {
        this.from = from;
        this.to = to;
        this.value = value;
    }
